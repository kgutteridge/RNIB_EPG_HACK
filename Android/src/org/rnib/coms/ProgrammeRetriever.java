...
public class ProgrammeRetriever {
	...
	static String dateRightNow = "201202121300";
	...
	interface Params {
		final String QUERY = "detail=2&dur=360&time=" + dateRightNow + "&channels=2002,2006,6000,1621,1801,1402,2201,1412,2617,2304,2306,2510,1842,2505,2061,2018,6240,6260,6272,2205,2207,4330,1305,2202,2501,3802,1813,1833,2203,6155,2303,2703,5701,1670,1628,3300,3340,3310,4075,2076,3618,1865,3352,3602,4610,3617,3207,1360,1832";
	}
	...
	private final ResultHandler channelResponseHandler = new ResultHandler() {
		...
		public void onSuccess(int resultCode, byte[] array) throws IOException {
			...
			ProgrammeResponse response = new Gson().fromJson(new String(array), ProgrammeResponse.class);  
	...
	private void retrieveProgramStream() {
		...
		dateRightNow = formatter.format(new Date());
		dateRightNow = dateRightNow + "00";
		...
		Intent intent = new ServiceIntentBuilder(
				...
				.setData(Uri.withAppendedPath(Uri.parse(BASE_URL), PROGS_API))
				.withParam(Params.QUERY, "")
